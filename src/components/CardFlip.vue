<template>
  <div class="card" :class="{disabled: isDisabled}" 
    :style="{
      ...
      perpective: `${((920 - 16 * 4) / Math.sqrt(cardsContext.length) - 16) * 3 / 4 * 2}px` }">
        <div class="card_inner" :class="{'is_flipped': isFlipped}" @click="onToggleFlipCard()">
          <div class="card_face card_face--front">
            <div class="card_content" :style="{backgroundSize: `${((920 - 16 * 4) / Math.sqrt(cardsContext.length) - 16) * 3 / 4 / 3}px ${((920 - 16 * 4) / Math.sqrt(cardsContext.length) - 16) * 3 / 4 / 3}px`}"></div>
          </div>
